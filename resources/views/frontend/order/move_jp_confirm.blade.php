    ...
    <div class="content">
      ...
      <h3>ドメイン移管情報</h3>
       <table class="form-input">
        ...
        <tr>
          <td class="title">公開連絡先</td>
          <td>{{ $confirmData['domain_public_contact'] }}</td>
        </tr>

        <!-- domain_organization_name -->
        <tr>
          <td class="title">登録担当者名</td>
          <td>
            @if ( $confirmData['domain_registered_name'] == 1 )
            チロロネットを窓口として公開する
            @elseif ( $confirmData['domain_registered_name'] == 2 )
            以下情報を窓口として公開する
            @elseif ( $confirmData['domain_registered_name'] == 3 )
            ご契約ご担当者を窓口として公開する
            @endif
          </td>
        </tr>
        ...
        <tr>
          ...
          <td>{{ $confirmData['domain_address_japan'] }}</td>
        </tr>
      ...
      <div style="float: left; width: 100%; margin: 50px 0 80px;">
        <div class="box-submit">
          <a href="{{ route('frontend.order.move_jp.send') }}" class="bt-submit">送信</a>
          <a href="{{ route('frontend.order.move_jp.back') }}" class="btn-back">戻る</a>
        <div>
      </div>
    </div>
  </div>
