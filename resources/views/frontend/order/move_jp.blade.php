...
<section class="order-child clear" id="contact">
  <div class="container">
    <h1 class="title-content">くるみサーバーお申込フォーム(汎用jpドメイン移管)</h1>

    <!-- form -->
    {!! Form::open(array('route' => 'frontend.order.move_jp.index', 'enctype'=>'multipart/form-data')) !!}
    <div class="content">
      ...
      <h3>ドメイン移管情報</h3>
       <table class="form-input">
        ...
        <!-- domain_public_contact -->
        <tr>
          <td class="title"><span>※ </span>公開連絡先</td>
          <td>
            @if ( isset($back['domain_public_contact']) )
            <input type="text" name="domain_public_contact" value="{{ $back['domain_public_contact'] }}" />
            @else
            <input type="text" name="domain_public_contact" value="{{ old('domain_public_contact') }}" />
            @endif
            @if ($errors->first('domain_public_contact'))<span class="error-input">{!! $errors->first('domain_public_contact') !!}</span>@endif
          </td>
        </tr>

        <!-- domain_registered_name -->
        <tr>
          <td class="title"><span>※ </span>登録担当者名</td>
          <td>
            <div>
              @if ( isset($back['domain_registered_name']) && $back['domain_registered_name'] == 1 )
              <input type="radio" name="domain_registered_name" value="1" checked="" /> チロロネットを窓口として公開する 
              @elseif ( old('domain_registered_name') == 1 )
              <input type="radio" name="domain_registered_name" value="1" checked="" /> チロロネットを窓口として公開する 
              @else
              <input type="radio" name="domain_registered_name" value="1" /> チロロネットを窓口として公開する 
              @endif
            </div>
            <div>
              @if ( isset($back['domain_registered_name']) && $back['domain_registered_name'] == 2 )
              <input type="radio" name="domain_registered_name" value="2" checked="" /> 以下情報を窓口として公開する 
              @elseif ( old('domain_registered_name') == 2 )
              <input type="radio" name="domain_registered_name" value="2" checked="" /> 以下情報を窓口として公開する 
              @else
              <input type="radio" name="domain_registered_name" value="2" /> 以下情報を窓口として公開する
              @endif
            </div>
            <div>
              @if ( isset($back['domain_registered_name']) && $back['domain_registered_name'] == 3 )
              <input type="radio" name="domain_registered_name" value="3" checked="" /> ご契約ご担当者を窓口として公開する 
              @elseif ( old('domain_registered_name') == 3 )
              <input type="radio" name="domain_registered_name" value="3" checked="" /> ご契約ご担当者を窓口として公開する 
              @else
              <input type="radio" name="domain_registered_name" value="3" /> ご契約ご担当者を窓口として公開する
              @endif
            </div>
            @if ($errors->first('domain_registered_name'))<span class="error-input">{!! $errors->first('domain_registered_name') !!}</span>@endif
          </td>
        </tr>

        <!-- domain_address_japan -->
        <tr>
          <td class="title"><span>※ </span>ご住所（日本語）</td>
          <td>
            @if ( isset($back['domain_address_japan']) )
            <input type="text" name="domain_address_japan" value="{{ $back['domain_address_japan'] }}" />
            @else
            <input type="text" name="domain_address_japan" value="{{ old('domain_address_japan') }}" />
            @endif
            @if ($errors->first('domain_address_japan'))<span class="error-input">{!! $errors->first('domain_address_japan') !!}</span>@endif
          </td>
        </tr>
      ...
      <h3>ご担当者について</h3>
       <table class="form-input">
        ...
        <tr>
          ...
          <td>
             〒
            @if ( isset($back['person_your_address_1']) )<input type="text" class="input-xs mr-bottom10" name="person_your_address_1" value="{{ $back['person_your_address_1'] }}" />
            @else<input type="text" class="input-xs" name="person_your_address_1" value="{{ old('person_your_address_1') }}" />
            @endif
             住所 
